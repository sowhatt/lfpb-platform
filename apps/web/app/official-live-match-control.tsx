...
type QuickEventType = 'GOAL' | 'YELLOW_CARD' | 'RED_CARD' | 'SUBSTITUTION' | 'INJURY' | 'INCIDENT';
...
const QUICK_EVENT_CONFIG: Record<QuickEventType, { label: string; spoken: string; hint: string }> = {
  GOAL: { label: '⚽ BUT', spoken: 'but', hint: 'Dites par exemple « numéro 8 Dragons »' },
  YELLOW_CARD: { label: '🟨 JAUNE', spoken: 'carton jaune', hint: 'Dites par exemple « numéro 5 Aziza »' },
  RED_CARD: { label: '🟥 ROUGE', spoken: 'carton rouge', hint: 'Dites par exemple « numéro 7 Dragons »' },
  SUBSTITUTION: { label: '🔄 REMPLACEMENT', spoken: 'remplacement', hint: 'Dites par exemple « 11 sort, 12 entre Dragons »' },
  INJURY: { label: '🩹 BLESSURE', spoken: 'blessure', hint: 'Dites par exemple « numéro 9 Aziza »' },
  INCIDENT: { label: '⚠ INCIDENT', spoken: 'incident', hint: 'Décrivez brièvement l’incident' },
};
...
export function OfficialLiveMatchControl({
 ...
 async function action(label:string,fn:()=>Promise<unknown>){setBusy(label);setError('');setMessage('');try{await fn();await refresh();onMatchUpdated?.();setManualMinute(false);setMessage(`${label} enregistré.`);}catch(reason){setError(reason instanceof Error?reason.message:`${label} impossible`);}finally{setBusy('');}} function post(path:string,body?:unknown){return request(path,token,{method:'POST',body:body===undefined?undefined:JSON.stringify(body)});} function postEvent(type:string,extra:Record<string,unknown>={}){const selectedMinute=manualMinute?minute:autoMinute;const eventMinute=type==='MATCH_START'?0:Math.max(0,Math.min(130,selectedMinute));const lifecycleEvent=['MATCH_START','HALF_TIME','SECOND_HALF_START','MATCH_END'].includes(type);const clockMetadata=manualMinute||lifecycleEvent?{}:{period:autoClock.period,stoppageMinute:autoClock.stoppageMinute};return post(`/matches/${matchId}/events`,{type,minute:eventMinute,...clockMetadata,...extra});}
 ...
 async function analyzeText(text:string,forcedMinute?:number){const value=text.trim();if(!value)return;setBusy('Analyse');setError('');setMessage('Analyse de la commande…');setDraft(null);try{const interpreted=await post('/official-assistant/interpretations',{transcript:value}) as VoiceDraft;if(interpreted.type==='NOTE')throw new Error('Commande non reconnue. Dites par exemple « carton rouge numéro 8 Dragons » ou précisez une minute.');if(interpreted.type==='FINAL_SCORE')throw new Error('Pour terminer la rencontre, utilisez le bouton « Fin du match ».');if(!allowedEventTypes.includes(interpreted.type))throw new Error(`${roleLabel} : cet événement n’est pas autorisé pour votre rôle sur cette rencontre.`);const eventMinute=forcedMinute??interpreted.minute??(manualMinute?minute:autoMinute);if(forcedMinute===undefined&&interpreted.minute!==undefined){setMinute(interpreted.minute);setManualMinute(true);}if(interpreted.type==='INCIDENT'||interpreted.type==='OBSERVATION'){setDraft({type:interpreted.type,minute:eventMinute,transcript:interpreted.transcript});setMessage(interpreted.minute===undefined?`Événement compris · minute automatique ${autoClock.label}'. Vérifiez puis confirmez.`:'Événement compris. Vérifiez puis confirmez.');return;}if(interpreted.type==='INJURY'&&interpreted.playerNumber===undefined)throw new Error('Le numéro du joueur blessé est nécessaire.');if(interpreted.playerNumber===undefined)throw new Error('Le numéro du joueur est nécessaire.');const player=resolvePlayer(interpreted.playerNumber,mentionedSide(value)),teamName=player.side==='HOME'?homeName:awayName,clubId=player.clubId;if(interpreted.type==='SUBSTITUTION'){if(interpreted.replacementPlayerNumber===undefined)throw new Error('Précisez le numéro du joueur entrant.');const secondaryPlayer=resolvePlayer(interpreted.replacementPlayerNumber,player.side);setDraft({type:'SUBSTITUTION',minute:eventMinute,transcript:interpreted.transcript,teamName,clubId,player,secondaryPlayer});setMessage('Événement compris à partir du contexte du match. Vérifiez puis confirmez.');return;}setDraft({type:interpreted.type,minute:eventMinute,transcript:interpreted.transcript,teamName,clubId,player});setMessage(interpreted.minute===undefined?`Événement compris · minute automatique ${autoClock.label}'. Vérifiez puis confirmez.`:'Événement compris à partir du contexte du match. Vérifiez puis confirmez.');}catch(reason){setMessage('');setError(reason instanceof Error?reason.message:'Analyse impossible');}finally{setBusy('');}}
 function selectQuickEvent(type:QuickEventType){const capturedMinute=manualMinute?minute:autoMinute;setQuickEvent(type);setQuickMinute(capturedMinute);setCommand('');setDraft(null);setError('');setMessage(`${QUICK_EVENT_CONFIG[type].label} · ${capturedMinute}e minute capturée. 🎤 Micro en préparation…`);setVoiceAutoStartKey((value)=>value+1);}
function quickTranscript(text:string){const value=text.trim();if(!quickEvent)return value;return `${QUICK_EVENT_CONFIG[quickEvent].spoken} ${value}`.trim();}
async function analyzeCommand(event:FormEvent<HTMLFormElement>){event.preventDefault();if(dictating){setError('La dictée est encore active. Attendez l’arrêt automatique après votre phrase, ou touchez « Terminer maintenant ».');return;}await analyzeText(quickTranscript(command),quickMinute??undefined);}
async function confirmDraft(){if(!draft)return;const payload:Record<string,unknown>={minute:draft.minute};if(draft.type==='INCIDENT'||draft.type==='INJURY'||draft.type==='OBSERVATION')payload.description=draft.transcript;if(draft.player&&draft.clubId){payload.clubId=draft.clubId;payload.registrationId=draft.player.registrationId;}if(draft.secondaryPlayer)payload.secondaryRegistrationId=draft.secondaryPlayer.registrationId;await action(eventLabels[draft.type]??draft.type,()=>postEvent(draft.type,payload));setDraft(null);setCommand('');setQuickEvent(null);setQuickMinute(null);} const liveReady=sheet?.status==='LOCKED'&&match?.status==='IN_PROGRESS';
 ...
 {match?.status==='IN_PROGRESS'&&<div style={{marginTop:20,padding:16,border:'2px solid #0f172a',borderRadius:14,background:'#f8fafc'}}>
<label>ÉVÉNEMENT RAPIDE · {manualMinute?minute:autoClock.label}'</label>
<h3 style={{marginTop:6}}>Que vient-il de se passer ?</h3>
<p>Choisissez l’événement. Digital Foot mémorise immédiatement la minute du chrono ; dites ensuite uniquement le joueur ou les informations manquantes.</p>
<div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(145px,1fr))',gap:10,marginTop:12}}>
{(Object.keys(QUICK_EVENT_CONFIG) as QuickEventType[]).filter((type)=>allowedEventTypes.includes(type)).map((type)=>{
const selected=quickEvent===type;
return <button key={type} type="button" disabled={!liveReady||Boolean(busy)||dictating} onClick={()=>selectQuickEvent(type)} style={{minHeight:64,fontSize:16,fontWeight:900,border:selected?'3px solid #0f172a':'1px solid #cbd5e1',background:selected?'#e2e8f0':'#fff',borderRadius:12}}>{QUICK_EVENT_CONFIG[type].label}</button>;
})}
</div>
{quickEvent&&<div className="draft-warning" style={{marginTop:12}}>
<strong>{QUICK_EVENT_CONFIG[quickEvent].label} · {quickMinute}'</strong>
<div style={{marginTop:4}}>{QUICK_EVENT_CONFIG[quickEvent].hint}</div>
<button type="button" style={{marginTop:8}} disabled={Boolean(busy)} onClick={()=>{setQuickEvent(null);setQuickMinute(null);setCommand('');setDraft(null);setMessage('');}}>Annuler l’événement rapide</button>
</div>}
</div>}
<div className="workspace-actions" style={{marginTop:20,display:'block'}}><label>ASSISTANT DIGITAL · {roleLabel.toUpperCase()}</label><h3>{quickEvent?`${QUICK_EVENT_CONFIG[quickEvent].label} · compléter l’événement`:assistantCopy.title}</h3><p>{quickEvent?QUICK_EVENT_CONFIG[quickEvent].hint:assistantCopy.description}</p><form onSubmit={analyzeCommand} style={{display:'flex',gap:10,flexWrap:'wrap',marginTop:12}}><input value={command} onChange={(event)=>{setCommand(event.target.value);setError('');setMessage('');setDraft(null);}} placeholder={quickEvent?QUICK_EVENT_CONFIG[quickEvent].hint:assistantCopy.placeholder} style={{flex:'1 1 420px'}}/><OfficialLiveTranscriber token={token} queueScope={`${matchId}:${assignmentRole}`} captureMinute={quickMinute??(manualMinute?minute:autoMinute)} autoStartKey={quickEvent?voiceAutoStartKey:undefined} disabled={Boolean(busy)} onListeningChange={setDictating} onDelta={(text)=>{setError('');setMessage('');setDraft(null);setCommand(text);}} onFinal={async(text,capturedMinute)=>{setCommand(text);await analyzeText(quickTranscript(text),quickMinute??capturedMinute);}} onError={(liveError)=>setError(liveError)}/><button type="submit" disabled={Boolean(busy)||dictating||!command.trim()}>{dictating?'Dictée en cours…':busy==='Analyse'?'Analyse…':'Analyser'}</button></form>{draft&&<div ref={draftRef} className="draft-warning" style={{marginTop:14,border:'2px solid #eab308'}}><strong>À CONFIRMER · {eventLabels[draft.type]}</strong><div style={{marginTop:6}}>{draft.teamName&&<span>{draft.teamName} · </span>}{draft.player&&<span>N°{draft.player.shirtNumber} · {playerName(draft.player)} · </span>}{draft.secondaryPlayer&&<span>entrant N°{draft.secondaryPlayer.shirtNumber} · {playerName(draft.secondaryPlayer)} · </span>}<span>{draft.minute}e minute</span></div>{!liveReady&&<p style={{marginTop:8}}>La confirmation sera disponible après validation, verrouillage de la feuille et coup d’envoi.</p>}<div style={{marginTop:10,display:'flex',gap:8}}><button type="button" disabled={!liveReady||Boolean(busy)} onClick={()=>void confirmDraft()}>✓ Confirmer et enregistrer</button><button type="button" disabled={Boolean(busy)} onClick={()=>setDraft(null)}>Annuler</button></div></div>}</div>
